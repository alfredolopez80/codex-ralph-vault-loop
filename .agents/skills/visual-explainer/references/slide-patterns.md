...
**Step 4 — Plan visuals.** Do not call external image generators from this skill. Use CSS gradients, inline SVG, Mermaid, charts, and approved existing images. Content slides with conceptual topics can use SVG or diagram treatments in the aside area.
...
Full-viewport hero. Background treatment via gradient, texture, inline SVG, or approved existing image. 80–120px display type.
...
Background image from an approved existing asset or CSS gradient dominates the viewport. Text overlay with gradient scrim ensuring contrast. Zero slide padding.
...
Slides should reach for visuals before defaulting to text alone. If a slide could be more compelling with an image, chart, or diagram, add one using CSS gradients, inline SVG, Mermaid, charts, or approved existing images. Do not call external image generators from this skill.
...
**When to skip images:** Pure structural or data-heavy decks (code reviews, table comparisons) may not need images. Use CSS gradients and SVG decorations instead.
