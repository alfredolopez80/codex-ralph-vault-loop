...
license: MIT
compatibility: Requires a browser to view generated HTML files. External publishing and non-OpenAI image generation are disabled in this sanitized Codex install.
metadata:
  author: nicobailon
  version: "0.6.3"
---
...
Generate self-contained HTML files for technical diagrams, visualizations, and data tables. Report the generated file path; open it in a browser only when the user explicitly asks or the current Codex environment provides an approved browser tool. Never fall back to ASCII art when this skill is loaded.

**Proactive table rendering.** When you're about to present tabular data as an ASCII box-drawing table in the terminal (comparisons, audits, feature matrices, status reports, any structured rows/columns), generate an HTML page instead. The threshold: if the table has 4+ rows or 3+ columns, it belongs in the browser. Don't wait for the user to ask — render it as HTML automatically and tell them the file path. You can still include a brief text summary in the chat, but the table itself should be the HTML page.

## Installed Scope

This sanitized Codex install is skill-only. It includes the core `SKILL.md`, templates, and references needed to generate local self-contained HTML. It does not install slash commands, prompt files, deployment scripts, or public sharing helpers.
...
**External image generation disabled.** This sanitized Codex install must not call `surf`, Gemini image generation, or any other external image generator from this skill. Use Mermaid, inline SVG, CSS, tables, and user-provided or approved local assets. If the user explicitly asks for generated imagery, use the current approved Codex image-generation route instead of this skill.
...
**Browser preview:** Do not run `open`, `xdg-open`, or other GUI commands automatically. If the user explicitly asks to open the file, use the approved Codex/browser route for the current environment.

**Tell the user** the file path so they can re-open it.
...
**Visual richness:** Do not call external image generators from this skill. Use SVG decorative accents, per-slide background gradients, inline sparklines, and small Mermaid diagrams. Visual-first, text-second.
...
Public sharing is disabled in this sanitized Codex install. Do not deploy generated pages to Vercel or any other public host from this skill. If the user asks to publish a page, stop and request explicit approval with the exact file path and hosting target.
...
- **File opens cleanly**: When preview is approved or available through the Codex browser route, verify no console errors, broken font loads, or layout shifts.
