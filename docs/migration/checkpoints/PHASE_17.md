...
`docs/migration/checkpoints/PHASE_16.md` was reviewed first. It is marked PASS, so Phase 17 was allowed to proceed.
...
The docs explain Codex App usage, checkpoint links, global activation expectations, memory boundaries, hooks, subagents, MCP routing, eval coverage, and threat controls. They state that there is no direct `model_provider` entry for Z.ai or MiniMax. They also state that Z.ai and MiniMax are not used for visual generation and that GPT Images 2 is the approved image generation route.

Manual validation:

```text
find docs -type f | sort
grep -R "direct .*model_provider" docs/migration docs/architecture
grep -R "GPT Images 2" docs/architecture AGENTS.md
```

Results: all required docs exist; direct `model_provider` language appears in migration and architecture docs; `GPT Images 2` appears in architecture docs. Additional checks confirmed that new docs link to checkpoint phases and explain Codex App usage.
