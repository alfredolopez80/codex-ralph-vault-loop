...
| Before command execution | `PreToolUse` / `pre_tool_guard.py`                                                                      | Enforce SFW, RED, context-budget, package-manager, and dangerous-command guardrails.                                                                | `bash .codex/tests/run-hook-tests.sh`; `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 python3 -m pytest tests/unit/test_context_budget.py -q`.                                                                            |
