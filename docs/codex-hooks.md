...
- `.codex/hooks/user_prompt_dispatch.py`
  - Owns the consolidated prompt context surface. Safety classification runs
    first, followed by active model provenance and a cheap new-store progress
    identity lookup.
  - Includes progress generation and context identity in the task signature,
    claims the content-free prompt cache before journal/history rendering, and
    returns empty on a hit or in-flight claim without a progress/checkpoint
    write. Recall and prompt improvement are miss-only components.
  - Emits a progress capsule only for an explicit progress request. Ordinary
    prompts never receive a rolling checkpoint plus a duplicate implementation
    narrative. Legacy prompt continuity remains an explicit compatibility mode.
