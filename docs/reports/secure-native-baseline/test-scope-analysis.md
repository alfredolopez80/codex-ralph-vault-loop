...
| **Migration evidence; must not enter `--minimal`**          | `tests/integration/test_hook_lifecycle_e2e.py`; `tests/unit/test_{session_start,user_prompt,post_tool,stop}_dispatch.py`; `tests/integration/test_{post_tool_checkpoint,post_tool_cost_ledger,session_start_sources_integration,progress_session_start_subprocess,stop_handoff_checkpoint}.py`                                                                                                                                                                                                                                                                                                                                                                                                                      | They exercise disabled lifecycle behavior or historical dispatchers. Preserve in standard/full while their later migration decisions remain open; do not add them to the fast security-only signal.                                           |
| **Retired in #85**                                          | Frozen execution authority, manual activation, authority leases/digests/phases, prompt-boundary gating, routing/advisor PreTool vetoes, and their dedicated unit/integration/canary suites.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | Removed only after the active security dispatcher proved independent. Active-security assertions were retained in `test_security_baseline.py`, graph tests, lockstep tests, and the broader security suite.                                   |
...
| **Belongs to #76 / #77 / #80 (retain pending issue scope)** | **#76:** `SessionStart`/`UserPromptSubmit` continuity and recovery (`test_session_start_dispatch.py`, `test_session_start_sources_integration.py`, `test_progress_session_start_subprocess.py`, `test_user_prompt_dispatch.py`, `test_effective_hook_chain.py` prompt-context case). **#77:** `PostToolUse`/`Stop` persistence and completion (`test_post_tool_dispatch.py`, `test_post_tool_checkpoint.py`, `test_post_tool_cost_ledger.py`, `test_stop_dispatch.py`, `test_stop_handoff_checkpoint.py`). **#80:** optional advisor state and worker reservation helpers that remain non-blocking (`test_sol_advisor_hooks.py`, `test_worker_reservation_lifecycle.py`, and relevant `test_maintenance_queue.py`). | Assignment is by behavior, not filename. These suites should stay available as issue-specific migration evidence until each issue states its final runtime contract and records focused evidence.                                             |
