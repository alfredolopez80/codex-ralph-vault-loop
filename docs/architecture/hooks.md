...
- `PostToolUse` blocks touched non-generated files over 350 lines, extracts memory candidates, and records cost ledger events, including route-family metadata.
- `Stop` rescans changed git files for the same 350-line guard, runs output quality checks, warns when non-trivial work lacks a visible `ROUTE_DECISION`, and persists a handoff.
