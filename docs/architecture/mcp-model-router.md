...
Codex main is the final owner. Z.ai and MiniMax are not direct completion providers. The phrase `model_provider directo` is intentionally rejected for both. The supported route is MCP tool use with a sensitivity check, intent-based routing decision, and Codex synthesis.
