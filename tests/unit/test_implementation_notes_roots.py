...
def test_guard_rejects_linked_plan_with_different_notes_path(tmp_path: Path) -> None:
    primary = make_repo(tmp_path)
    active = add_worktree(primary, tmp_path / "ordinary-linked" / "active-copy")
    active_plan = active / ".ralph" / "plans" / "nested" / "feature.md"
    canonical_plan = primary / ".ralph" / "plans" / "nested" / "feature.md"
    ...
    active_plan.write_text(
        f"# Feature\n\nImplementation notes: {primary / '.ralph/plans/nested/other.html'}\n",
        encoding="utf-8",
    )
    canonical_plan.write_text(
        f"# Feature\n\nImplementation notes: {primary / '.ralph/plans/nested/feature-implementation-notes.html'}\n",
        encoding="utf-8",
    )

    roots = resolve_roots(active_root=active)

    with pytest.raises(ImplementationNotesError, match="different notes path"):
        canonical_plan_for_guard(active_plan, roots)
