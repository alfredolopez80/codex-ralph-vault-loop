...
This skill covers the JSON shape Skottie needs. For timing, easing,
choreography, and Disney animation principles, use LottieFiles'
[motion-design skill](https://github.com/lottiefiles/motion-design-skill).
That guidance uses milliseconds; convert with `frames = ms / 1000 * fr`.
...
Always use the official GitHub player project. The rules below depend on that
project's slots, properties panel, `?frame=` controls, and Skottie wasm wiring.
Do not build a separate HTML page, swap in `lottie-web`, or scaffold a bespoke
canvas setup; those paths render differently and invalidate the verification
steps. If the player project isn't already on this machine, scaffold a fresh
copy of the audited repo revision with **degit**:

```bash
sfw npx degit diffusionstudio/lottie#ce3d2a0d180b6ad4d8fa40f53c7a3e103ee72790 my-animation
cd my-animation
sfw npm install   # postinstall copies the CanvasKit wasm into /public
npm run dev
```

Keep the scaffold revision pinned. If you need a newer upstream player, inspect
the upstream diff first, update the commit in this command, and rerun the
security and render checks.

Then open the printed local URL. If you already have the project, run
`sfw npm install` when dependencies need to be fetched or refreshed, then
`npm run dev`.

## Where to write the file (and how it loads)

- Write the animation JSON to **`public/lottie.json`**. That is the only file
  you need to touch to change what the app shows; `src/App.tsx` fetches
  `/lottie.json` at startup.
...
Prefer **shape layers (`ty: 4`)** for LLM-authored animations. They need no
external assets.
...
list of shapes and fills directly in `shapes` renders **blank**. Always nest the
geometry, fill or stroke, and a group transform inside a group's `it` array:
...
Shape primitives inside `it`: `"el"` is an ellipse with `p` center and `s`
size; `"rc"` is a rectangle with `p` center, `s` size, and `r` corner radius;
`"sh"` is a custom bezier path in `ks.k`; `"st"` is a stroke; `"fl"` is a fill;
and `"tr"` is the group's transform. Always include `"tr"` last.
...
edit chosen values of the animation in real time. This uses Skottie's native
**slot** feature, so changes show on the next frame without a re-parse.

To make a property editable, do two things:

### 1. Declare a slot in the Lottie JSON

Add a top-level `"slots"` object whose
...
| Slot value | Slot definition shape | Control rendered |
|------------|-----------------------|------------------|
| scalar (a single number) | `{ "p": { "a": 0, "k": 120 } }` | slider |
| color (RGBA 0–1) | `{ "p": { "a": 0, "k": [1, 1, 1, 1] } }` | color picker |
| vec2 (`[x, y]`) | `{ "p": { "a": 0, "k": [256, 256] } }` | two number inputs |
| text | `{ "p": { "p": { "t": "Label" } } }` | text input |

Text slots are the one special case: the player reads and exports text values
at `slots.<sid>.p.p.t`, not as a raw string or scalar under `p.k`. Use the
nested text-property shape above so Skottie exposes the slot as a text control
and the downloaded configured Lottie preserves edits.
...
Every animation you produce must expose at least one control for the background
color. The player does not paint a composition background of its own, so add a
full-composition background layer as the **last** entry in `layers`, fill it
with a slotted color, and label that slot in `controls.json`. Use a rectangle
...
The layer position centers the shape layer at `[w / 2, h / 2, 0]`; keep the
rectangle centered at `[0, 0]` inside that layer so the offsets do not add
together. Match the rectangle's `s` to your composition's `w`×`h`. This is in
addition to whatever other controls the animation exposes.

### 2. Describe presentation in `public/controls.json` (optional)

Slots expose an ID and type. The sidecar file adds labels and slider ranges.
Missing entries fall back to the slot ID and a generic 0-100 range. Like
`lottie.json`, it hot-reloads on save.
...
hunt for the play button. Those actions are unreliable and cannot land on an
exact frame. Pin the frame in the URL and read the canvas by its test id:
...
To change the inspected frame, open a new URL or edit the query string and
reload. The canvas carries `data-testid="lottie-canvas"`, so a
...
8. If the dev server is running, save the file and use hot reload. Otherwise
   start it with `npm run dev`. A blank canvas with no error usually means the
   group wrapping needs another check.
